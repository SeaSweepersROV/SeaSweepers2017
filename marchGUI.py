...
#baudRate = 9600
# = Serial(serialPort , baudRate, timeout=0, writeTimeout=0) #ensure non-blocking, code will not run if the port is not connected
...
class App():
	...
	def __init__(self):
		...
		self.outerTempFLabel = tk.Label(fg="white",bg="black",text="Outer Temp.(F)",relief=tk.GROOVE,width=17,height=2).grid(                        column=0,row=2,columnspan=1)
		self.innerTempFLabel = tk.Label(fg="white",bg="black",text="Inner Temp. (F)",relief=tk.GROOVE,width=17,height=2).grid(                       column=0,row=3,columnspan=1)
		self.pressureLabel = tk.Label(fg="white",bg="black",text="Pressure(pa)",relief=tk.GROOVE,width=17,height=2).grid(                            column=0,row=4,columnspan=1)
